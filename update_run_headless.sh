...
sudo docker compose pull valheim_update &>/dev/null
#ensure steamcmd image is latest. It updates frequently
#https://hub.docker.com/r/steamcmd/steamcmd

sudo docker compose stop valheim_srv &>/dev/null
#ensure server is not running

sudo docker compose up valheim_update &>/dev/null
#check for and apply any updates
#container will terminate when update is complete

sudo docker compose up -d valheim_srv &>/dev/null
